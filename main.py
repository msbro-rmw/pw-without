...
async def start(bot, message):
  ...
  keyboard = [
    [
      InlineKeyboardButton("🚀 Physics Wallah withouts Purchase🙈🚀", callback_data="pwwp")
    ],
    [
...
async def process_pwwp(bot: Client, m: Message, user_id: int):
    ...
    # FIX: Separate headers for OTP and Token requests (from JS reference)
    otp_headers = {
    ...
    # General headers for all other PW API calls after login
    headers = {
        'Host': 'api.penpencil.co',
        'Content-Type': 'application/json',
        'Client-Id': '5eb393ee95fab7468a79d189',
        ...
        try:
            if raw_text1.isdigit() and len(raw_text1) == 10:
                ...
                try:
                    ...
                    async with session.post(f"https://api.penpencil.co/v1/users/get-otp?smsType=0", json=data, headers=otp_headers) as response:
                        resp_data = await response.json()
                        if not resp_data.get("success", True) and resp_data.get("message"):
                            await editable.edit(f"**OTP Error : {resp_data.get('message')}**")
                            return
                ...
                try:
                    # FIX: Use token_headers (dynamic Randomid, Referer, correct UA, Integration-With: '')
                    token_headers['Randomid'] = str(uuid.uuid4())  # Fresh UUID for each token request
                    async with session.post(f"https://api.penpencil.co/v3/oauth/token", json=payload, headers=token_headers) as response:
                        resp_json = await response.json()
                        # FIX: Proper OTP error detection (isOTPErr equivalent from JS)
                        err_str = str(resp_json).lower()
                        if any(x in err_str for x in ['invalid otp', 'otp expired', 'wrong otp', 'incorrect otp']):
                            await editable.edit(f"**Error : Wrong or Expired OTP ❌ Please try again**")
                            return
