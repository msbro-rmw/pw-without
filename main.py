...
async def start(bot, message):
  ...
  await message.reply_photo(
    ...
    caption="**Hello PLEASE👇PRESS👇HERE**",
    quote=True,
...
def build_pw_video_url(videoUrl: str, video_details: dict, data_item: dict) -> str:
    """
    PW video URL mein parentId aur childId append karo.
    
    Final format:
      https://d1d34p8vz63oiq.cloudfront.net/<uuid>/master.mpd&parentId=<scheduleId>&childId=<videoId>
    
    - parentId  = schedule ka _id  (data_item['_id'])
    - childId   = videoDetails ka _id  (video_details['_id'])
    
    Ye dono IDs DRM licence request ke liye zaroori hain.
    Agar kisi ek ki value missing ho to URL as-is return karo taaki kuch toh mile.
    """
    parent_id = data_item.get('_id', '')
    child_id  = video_details.get('_id', '')

    if parent_id and child_id:
        # Already has query params? use & else use &
        separator = '&' if '?' in videoUrl else '&'
        return f"{videoUrl}{separator}parentId={parent_id}&childId={child_id}"
    return videoUrl
    ...
    if data and data.get("success") and data.get("data"):
        ...
        if content_type in ("videos", "DppVideos"):
            ...
            if video_details:
                ...
                if videoUrl:
                    # parentId & childId append karo — DRM ke liye zaroori
                    videoUrl = build_pw_video_url(videoUrl, video_details, data_item)
